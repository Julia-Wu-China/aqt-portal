...
**脚本原理（重要，别改坏它）：**
- 当前文件夹是**企微微盘同步目录**，WeDrive 会在 `.git/refs` 放占位文件导致 git 引用写不进去——**微盘文件夹里不能直接跑 git**。
- 所以脚本用**中转模式**：把改好的文件同步到本地隐藏仓库 `C:\Users\Administrator\.aqt-portal-git\`（首次自动 clone，之后自动对齐远端），在那里提交并 push，触发 Render 部署。
- `备份包/`、`data/`、`.env`、`node_modules` 会自动跳过，不会被推送。
- 推送需要 GitHub 凭据：首次会弹登录窗口（登录 Julia-Wu-China 账号）；失败则向门户管理员要 Token。
- 你**修改/新增/删除的任何代码文件**（index.html、server.js、functions/ 等）都会被同步上去，改完直接跑这一条即可。
